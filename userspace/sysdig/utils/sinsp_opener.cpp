...
void sinsp_opener::open(sinsp* inspector, unsigned long driver_buffer_bytes_dim) const
{
    if(driver_buffer_bytes_dim == 0)
    {
        throw sinsp_exception("the size specified for the --buffer_dim option is too small.");
    }
    ...
    if(bpf.enabled)
    {
		...
		if(bpf.modern)
		{
			inspector->open_modern_bpf(driver_buffer_bytes_dim, bpf.cpus_for_each_syscall_buffer, true, sc_of_interest);
			return;
		}
#endif
        ...
        try
        {
            inspector->open_bpf(probe, driver_buffer_bytes_dim, sc_of_interest);
        }
    ...
    try
    {
        inspector->open_kmod(driver_buffer_bytes_dim, sc_of_interest);
    }
    catch(const sinsp_exception& e)
    {
