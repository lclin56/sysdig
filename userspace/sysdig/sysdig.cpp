...
" --ring_buffer_dim <dim> Dimension in MB that every per-CPU buffer will have. Default 8MB.\n"
...
sysdig_init_res sysdig_init(int argc, char **argv)
{
	...
	static struct option long_options[] =
	{
		...
		{"ring_buffer_dim", required_argument, 0, 0 },
		{0, 0, 0, 0}
	};
	...
	try
	{
			...
			case 0:
				{
					...
					else if (optname == "ring_buffer_dim")
					{
						ring_buffer_dim_mb = atoi(optarg);
						if(ring_buffer_dim_mb <= 0)
						{
							throw sinsp_exception(std::string("invalid ring buffer dim") + optarg);
							res.m_res = EXIT_FAILURE;
							goto exit;
						}
					}
		...
		for(uint32_t j = 0; j < infiles.size() || infiles.empty(); j++)
		{
			...
			if(!infiles.empty())
			{
				...
				opener.open(inspector.get(), ring_buffer_dim_mb * 1024 * 1024);
			}
			else
			{
				if(j > 0)
